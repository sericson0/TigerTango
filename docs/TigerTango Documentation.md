...
![alt text](<Installing Development Branch-1.png>)
...
![alt text](<Setting up audio.png>)

### Layout

![alt text](<TangoTiger Breakdown.png>)
...
!!!WARNING !WARNING! If a deck is playing, do not click auto on the other deck. THIS WILL SKIP TO THE NEXT SONG. A future update will put a check to avoid this possibility, but for now, make sure to only click Auto when no decks are playing or on a currently playing deck.
...
!!!WARNING Make sure your Automix Type is set to "None (back-to-back)". If you want Automix to remove songs from the list once played then toggle "Auto remove played".
...
!!!WARNING The automix can be finicky at times in VirtualDJ. Make sure to click Auto on the track that you want to play from.
